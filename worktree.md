...
|단계|명령어|설명|
|--|--|--|
|1. 폴더 이동|cd ..|워크트리 폴더 안에서는 삭제가 불가능하므로 상위 폴더로 이동합니다.|
|2. 워크트리 삭제|git worktree remove <경로>|워크트리 등록을 해제하고 해당 폴더를 물리적으로 삭제합니다.|
|3. 상태 확인|git worktree list|현재 남아있는 워크트리 목록을 확인합니다.|
|4. 강제 정리|git worktree prune|"(선택) 폴더를 수동으로 지웠을 경우, 연결된 관리 정보를 강제로 정리합니다."|
